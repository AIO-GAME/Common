...
def parse_arguments():  # 获取传入的参数
    ...
    parser.add_argument('-m', '--msgtype', type=str, required=False, help='钉钉群机器人的access_token',
                        choices=['text', 'link', 'markdown', 'actionCard', 'feedCard'], default='text')
...
try:
    ...
    data = json.dumps(msg_content).encode('utf-8')
    ...
    with urllib.request.urlopen(req) as response:
        ...
        if 'errcode' in result and result['errcode'] != 0:
            ...
            print(f'钉钉消息发送失败，返回结果：{result}')
            sys.exit(exitCode)
        else:
            print(f'钉钉消息发送成功，返回结果：{result}')
            exitCode = 0
            sys.exit(exitCode)

...
namespace AIO.RainbowFolders
{
    internal class RainbowFoldersPopup : CoreDraggablePopupWindow
    {
        ...
        public static RainbowFoldersPopup GetDraggableWindow() { return CoreDraggablePopupWindow.GetDraggableWindow<RainbowFoldersPopup>(); }
        ...
        private static void DrawPreview(ref Rect rect, ProjectRule rule)
        {
            ...
            GUI.DrawTexture(image: !rule.HasLargeIcon()
                                ? ProjectEditorUtility.GetDefaultFolderIcon()
                                : rule.HasCustomIcon()
                                    ? rule.LargeIcon
                                    : ProjectIconsStorage.GetIcons(rule.IconType).Item1,
                            position: rect);
            rect.y += 44f;
            // num3   =  (rect.width = (rect.height = 16f));
            GUI.DrawTexture(image: !rule.HasSmallIcon()
                                ? ProjectEditorUtility.GetDefaultFolderIcon()
                                : rule.HasCustomIcon()
                                    ? rule.SmallIcon
                                    : ProjectIconsStorage.GetIcons(rule.IconType).Item2,
                            position: rect);
            rect.y     += 19f;
            rect.width =  64f;
            if (rule.HasBackground())
            {
                GUI.DrawTexture(rect, rule.HasCustomBackground() ? rule.BackgroundTexture : CoreBackgroundsStorage.GetBackground(rule.BackgroundType));
            }
        ...
        private void DrawSeparators(ref Rect rect)
        {
            ...
            rect.y = position.height - 16f - 11f;
            EditorGUI.DrawRect(rect, color);
            rect.y += 1f;
            EditorGUI.DrawRect(rect, color2);
        }
        ...
        private void ChangeWindowSize(bool hasCustomIcon, bool hasCustomBackground)
        {
            ...
            if (Mathf.Approximately(num3, rect.height))
            {
                rect.height            = num3;
                position               = rect;

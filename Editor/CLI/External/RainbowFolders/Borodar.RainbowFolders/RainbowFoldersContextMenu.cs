...
namespace AIO.RainbowFolders
{
    internal static class RainbowFoldersContextMenu
    {
        ...
        public static void RevertToDefault() { RevertSelectedFoldersToDefault(); }

        [MenuItem("Assets/Rainbow Folders/All Rules", false, 2400)]
        public static void ShowAllRules() { ProjectRuleset.ShowInspector(); }
        ...
        {
            Selection.assetGUIDs.ToList()
                     .ForEach(assetGuid =>
                     {
                         var text = AssetDatabase.GUIDToAssetPath(assetGuid);
                         if (!AssetDatabase.IsValidFolder(text)) return;
                         var assetPath = AssetDatabase.GetAssetPath(AssetDatabase.LoadAssetAtPath<DefaultAsset>(text));

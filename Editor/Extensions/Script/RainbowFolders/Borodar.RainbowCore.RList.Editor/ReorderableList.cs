...
namespace Borodar.RainbowCore.RList.Editor
{
	public class ReorderableList
	{
		...
		private static class Style
		{
			...
			static Style()
			{
				...
				iconPagePopup = EditorGUIUtility.IconContent("UnityEditor.HierarchyWindow", "| Select page");
				...
				verticalLabel.contentOffset = new Vector2(10f, 3f);
		...
		private struct DragList
		{
			...
			internal void SortByIndex()
			{
				Array.Sort(elements, delegate(DragElement a, DragElement b)
				{
					...
					return a.selected ? ((!b.selected) ? (-1) : b.startIndex.CompareTo(a.startIndex)) : a.startIndex.CompareTo(b.startIndex);
				});
			}
			...
			{
				Array.Sort(elements, (DragElement a, DragElement b) => a.desiredRect.center.y.CompareTo(b.desiredRect.center.y));
			}

			internal int GetIndexFromSelection(int index)
			{
				return Array.FindIndex(elements, (DragElement t) => t.startIndex == index);
			}
		}
		...
		private struct DragElement
		{
			...
			internal void RecordState()
			{
				...
				Iterate(this, property, delegate(DragElement e, SerializedProperty p, int index)
				{
			...
			private static void Iterate(DragElement element, SerializedProperty property, Action<DragElement, SerializedProperty, int> action)
			{
				...
				int num = 0;
				while (serializedProperty.NextVisible(enterChildren: true) && !SerializedProperty.EqualContents(serializedProperty, endProperty))
				{
		...
		private struct Pagination
		{
			...
			internal int GetPageForIndex(int index)
			{
				...
				return Mathf.FloorToInt((float)index / (float)pageSize);
			}
			...
			internal int GetPageCount(int total)
			{
				...
				return Mathf.CeilToInt((float)total / (float)pageSize);
			}
		...
		private class ListSelection : IEnumerable<int>, IEnumerable
		{
			...
			internal void RevertValues(object userData)
			{
				...
				for (int i = 0; i < Length; i++)
				{
					SerializedProperty arrayElementAtIndex = serializedProperty.GetArrayElementAtIndex(this[i]);
		...
		private static class ListSort
		{
			...
			internal static void SortOnProperty(SerializedProperty list, int length, bool descending, string propertyName)
			{
				BubbleSort(list, length, delegate(SerializedProperty p1, SerializedProperty p2)
				{
					...
					if (serializedProperty != null && serializedProperty2 != null && serializedProperty.propertyType == serializedProperty2.propertyType)
					{
						int num = Compare(serializedProperty, serializedProperty2, descending, serializedProperty.propertyType);
						if (!descending)
						{
							return num;
						}
		...
		private static class Internals
		{
			...
			static Internals()
			{
				dragDropValidation = Type.GetType("UnityEditor.EditorGUI, UnityEditor").GetMethod("ValidateObjectFieldAssignment", BindingFlags.Static | BindingFlags.NonPublic);
				appendDragDrop = typeof(SerializedProperty).GetMethod("AppendFoldoutPPtrValue", BindingFlags.Instance | BindingFlags.NonPublic);
			}

			internal static UnityEngine.Object ValidateObjectDragAndDrop(UnityEngine.Object[] references, SerializedProperty property, Type type, bool exactType)
			{
		...
		public int Length
		{
			get
			{
				...
				UnityEngine.Object[] targetObjects = list.serializedObject.targetObjects;
				for (int i = 0; i < targetObjects.Length; i++)
				{
					num = Mathf.Min(new SerializedObject(targetObjects[i]).FindProperty(list.propertyPath).arraySize, num);
				}
				return num;
			}
		}
		...
		public float GetHeight()
		{
			if (HasList)
			{
				float num = (doPagination ? (headerHeight + paginationHeight) : headerHeight);
				if (!list.isExpanded)
				{
					return headerHeight;
				}
				return num + GetElementsHeight() + footerHeight;
			}
		...
		public void DoList(Rect rect, GUIContent label)
		{
			...
			if (!HasList)
			{
				DrawEmpty(rect2, $"{label.text} is not an Array!", GUIStyle.none, EditorStyles.helpBox);
			}
			else
			{
				...
				if (list.isExpanded)
				{
					...
					if (selection.Length > 1 && current.type == EventType.ContextClick && CanSelect(current.mousePosition))
					{
		...
		public void RemoveItem(int index)
		{
			if (index >= 0 && index < Length)
			{
				...
				if (arrayElementAtIndex.propertyType == SerializedPropertyType.ObjectReference && (bool)arrayElementAtIndex.objectReferenceValue)
				{
		...
		private float GetElementHeight(SerializedProperty element)
		{
			...
			float propertyHeight = EditorGUI.GetPropertyHeight(element, GetElementLabel(element, elementLabels), IsElementExpandable(element));
		...
		private void DrawHeader(Rect rect, GUIContent label)
		{
			...
			bool flag = elementDisplayType != ElementDisplayType.SingleLine;
			Rect rect2 = rect;
			rect2.xMin += 6f;
			rect2.xMax -= (flag ? 95f : 55f);
			rect2.height -= 2f;
			rect2.y++;
			label = EditorGUI.BeginProperty(rect2, label, list);
			if (this.drawHeaderCallback != null)
			{
				this.drawHeaderCallback(rect2, label);
			}
			else if (expandable)
			{
				rect2.xMin += 10f;
				EditorGUI.BeginChangeCheck();
				bool isExpanded = EditorGUI.Foldout(rect2, list.isExpanded, label, toggleOnLabelClick: true);
			...
			{
				GUI.Label(rect2, label, EditorStyles.label);
			}
		...
		private void DrawElements(Rect rect, Event evt)
		{
			...
			if (!dragging)
			{
				...
				for (int i = start; i < end; i++)
				{
					...
					DrawElement(list.GetArrayElementAtIndex(i), GetElementDrawRect(i, elementRects[i]), flag, flag && GUIUtility.keyboardControl == controlID);
				}
			}
			else
			{
		...
		private void DrawElement(SerializedProperty element, Rect rect, bool selected, bool focused)
		{
			if (!(rect.height < 1f))
			{
				...
				else if (current.type == EventType.Repaint)
				{
					...
					EditorGUI.DrawRect(new Rect(rect.x + 1f, rect.y + rect.height - 2f, rect.width - 3f, 1f), new Color(0f, 0f, 0f, 0.2f));
				}
				if (current.type == EventType.Repaint && draggable)
				{
					Style.draggingHandle.Draw(new Rect(rect.x + 5f, rect.y + 14f, 10f, rect.height - (rect.height - 6f)), isHover: false, isActive: false, on: false, hasKeyboardFocus: false);
				}
		...
		private GUIContent GetElementLabel(SerializedProperty element, bool allowElementLabel)
		{
			...
			string text = ((this.getElementNameCallback == null) ? GetElementName(element, elementNameProperty, elementNameOverride) : this.getElementNameCallback(element));
		...
		private void DrawFooter(Rect rect)
		{
			...
			Rect rect2 = new Rect(rect.xMin + 4f, rect.y - 3f, 25f, 13f);
			Rect position = new Rect(rect.xMax - 29f, rect.y - 3f, 25f, 13f);
			EditorGUI.BeginDisabledGroup(!canAdd);
			if (GUI.Button(rect2, (this.onAddDropdownCallback != null) ? Style.iconToolbarPlusMore : Style.iconToolbarPlus, Style.preButton))
			{
			...
			EditorGUI.BeginDisabledGroup(!CanSelect(selection) || !canRemove || (this.onCanRemoveCallback != null && !this.onCanRemoveCallback(this)));
		...
		private void DrawPaginationHeader(Rect rect)
		{
			...
			int num = Mathf.Clamp(pagination.page, 0, pageCount - 1);
			if (num != pagination.page)
			{
				pagination.page = num;
				HandleUtility.Repaint();
			}
			Rect position = new Rect(rect.xMin + 4f, rect.y - 1f, 17f, 14f);
			Rect position2 = new Rect(position.xMax, rect.y - 1f, 17f, 14f);
			Rect position3 = new Rect(position2.xMax, rect.y - 1f, 17f, 14f);
			if (Event.current.type == EventType.Repaint)
			{
			...
			if (EditorGUI.DropdownButton(position2, Style.iconPagePopup, FocusType.Passive, Style.preButton))
			{
				...
				{
					int num2 = i;
					genericMenu.AddItem(new GUIContent($"Page {i + 1}"), i == pagination.page, OnPageDropDownSelect, num2);
				}
		...
		private void HandleSingleContextClick(Event evt, SerializedProperty element)
		{
			...
			{
				genericMenu.AddItem(new GUIContent(string.Concat("Revert ", GetElementLabel(element, allowElementLabel: true).text, " to Prefab")), on: false, selection.RevertValues, list);
				genericMenu.AddSeparator(string.Empty);
			}
			HandleSharedContextClick(evt, genericMenu, "Duplicate Array Element", "Delete Array Element", "Move Array Element");
		}
		...
		private void HandleMultipleContextClick(Event evt)
		{
			...
			HandleSharedContextClick(evt, genericMenu, "Duplicate Array Elements", "Delete Array Elements", "Move Array Elements");
		}
		...
		private void HandleMoveElement(object userData)
		{
			...
			List<DragElement> list = new List<DragElement>(dragList.Elements.Where((DragElement t) => !selection.Contains(t.startIndex)));
			selection.Sort();
		...
		private void HandleDragAndDrop(Rect rect, Event evt)
		{
			...
			case EventType.DragPerform:
			{
				...
				{
					UnityEngine.Object @object = (array[0] = array2[i]);
					UnityEngine.Object object2 = ValidateObjectDragAndDrop(array);
					if (object2 != null)
					{
		...
		private void HandlePostSelection(Rect rect, Event evt)
		{
			...
			case EventType.MouseDown:
				if (rect.Contains(evt.mousePosition) && IsSelectionButton(evt))
				{
					...
					{
						DoSelection(selectionIndex, GUIUtility.keyboardControl == 0 || GUIUtility.keyboardControl == controlID || evt.button == 2, evt);
					}
					else
					{
						selection.Clear();
			...
			case EventType.MouseUp:
				if (!draggable)
				{
					...
					if (this.onMouseUpCallback != null && IsPositionWithinElement(evt.mousePosition, selection.Last))
					{
				...
				else if (GUIUtility.hotControl == controlID)
				{
					...
					{
						dragging = false;
						ReorderDraggedElements(dragDirection, dragList.StartIndex, delegate
						{
							dragList.SortByPosition();
						});
					}
					else
					{
		...
		private void ReorderDraggedElements(int direction, int offset, Action sortList)
		{
			...
			selection.Sort(delegate(int a, int b)
			{
				...
				return (direction <= 0) ? indexFromSelection3.CompareTo(indexFromSelection2) : indexFromSelection2.CompareTo(indexFromSelection3);
			});
			int num = selection.Length;
			while (--num > -1)
			{

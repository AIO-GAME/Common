    ...
    public class ProjectRuleset : ScriptableObject
    {
            ...
            get
            {
                if (_instance == null)
                {
                    if (paths is null)
                    {
                        ...
                        foreach (var guid in AssetDatabase.FindAssets($"t:{nameof(ProjectRuleset)}", new string[] { "Packages" }))
                        {
                    ...
                    foreach (var expr in paths)
                    {
                        ...
                        if (_instance != null)
                        {
                            ...
                            OnRulesetChange = (Action)Delegate.Combine(OnRulesetChange, new Action(_instance.UpdateOrdinals));
                            _instance.UpdateDictionaries();
                            OnRulesetChange = (Action)Delegate.Combine(OnRulesetChange, new Action(_instance.UpdateDictionaries));
                            return _instance;
                        }
                    }

                    // _instance = AssetDatabase.LoadAssetAtPath<ProjectRuleset>("Assets/Plugins/RainbowAssets/RainbowFolders/Editor/Data/RainbowFoldersRuleset.asset");
                    // if (_instance != null)
                    // {
                    //     _instance.UpdateOrdinals();
                    //     OnRulesetChange = (Action)Delegate.Combine(OnRulesetChange, new Action(_instance.UpdateOrdinals));
        ...
        public static void ShowInspector(DefaultAsset asset = null)
        {
            ...
            EditorApplication.delayCall = (EditorApplication.CallbackFunction)Delegate.Combine(EditorApplication.delayCall, (EditorApplication.CallbackFunction)delegate
            {
                EditorApplication.delayCall = (EditorApplication.CallbackFunction)Delegate.Combine(EditorApplication.delayCall, (EditorApplication.CallbackFunction)delegate
                {
                    foreach (ProjectRulesetEditor eDITOR in ProjectRulesetEditor.EDITORS)
                    {
                        eDITOR.Asset = asset;
        ...
        public ProjectRule GetRule(ProjectRule match)
        {
            ...
            return Rules.Find((ProjectRule x) => x.Type == match.Type && x.Key == match.Key);
        }
        ...
        public ProjectRule GetRuleByPath(string folderPath, bool allowRecursive = false)
        {
            ...
            string assetNameFromPath = GetAssetNameFromPath(folderPath);
            ProjectRule result = null;
            if (allowRecursive)
            {
                foreach (ProjectRule item in _rulesByNameRecursive.Where((ProjectRule nameRecursive) => folderPath.Contains(string.Concat("/", nameRecursive.Key, "/"))))
                {
            ...
            if (allowRecursive)
            {
                foreach (ProjectRule item2 in _rulesByPathRecursive.Where((ProjectRule pathRecursive) => folderPath.StartsWith(string.Concat(pathRecursive.Key, "/"))))
                {
        ...
        public void RemoveAll(ProjectRule match)
        {
            ...
            {
                Undo.RecordObject(this, "Modify Rainbow Folder Settings");
                Rules.RemoveAll((ProjectRule x) => x.Type == match.Type && x.Key == match.Key);
                SaveSetting();
            }
        }
        ...
        {
            Undo.RecordObject(this, "Modify Rainbow Folder Settings");
            ProjectRule projectRule = Rules.SingleOrDefault((ProjectRule x) => x.Type == value.Type && x.Key == value.Key);
            if (projectRule == null)
            {

...
{
    internal static class ProjectEditorUtility
    {
        ...
        public static T LoadFromAsset<T>(string relativePath)
        where T : Object
        {
            var text = Path.Combine(ProjectPreferences.HomeFolder, relativePath);
            var val  = AssetDatabase.LoadAssetAtPath<T>(text);
            if (!val)
            {
        ...
        public static Texture2D GetSettingsButtonIcon() { return GetCoreTexture(ref _settingsIcon, CoreEditorTexture.IcnSettings); }

        public static Texture2D GetFilterButtonIcon() { return GetCoreTexture(ref _filterIcon, CoreEditorTexture.IcnFilter); }

        public static Texture2D GetDeleteButtonIcon() { return GetCoreTexture(ref _deleteIcon, CoreEditorTexture.IcnDelete); }

        public static Texture2D GetFoldoutIcon() { return GetCoreTexture(ref _foldoutIcon, CoreEditorTexture.IcnFoldoutMiddle); }

        public static Texture2D GetFoldoutLevelsIcon() { return GetCoreTexture(ref _foldoutLevelsIcon, CoreEditorTexture.IcnFoldoutLevels); }

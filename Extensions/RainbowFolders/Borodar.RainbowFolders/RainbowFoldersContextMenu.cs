...
namespace Borodar.RainbowFolders
{
	public static class RainbowFoldersContextMenu
	{
		...
		public static void ApplyCustom()
		{
			...
			Vector2 inPosition = ProjectWindowAdapter.GetFirstProjectWindow().position.position + new Vector2(10f, 30f);
			List<string> source = Selection.assetGUIDs.Select(AssetDatabase.GUIDToAssetPath).Where(AssetDatabase.IsValidFolder).ToList();

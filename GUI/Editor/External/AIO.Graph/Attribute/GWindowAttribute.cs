...
namespace AIO.UEditor
{
    ...
    public sealed class GWindowAttribute : DisplayNameAttribute
    {
        ...
        /// 停靠类型
        /// </summary>
        public Type[] DockType => Array.ConvertAll(Dock, Type.GetType);
        ...
        public GUIContent GetTitle()
        {
            if (!string.IsNullOrEmpty(IconRelative))
                Title.image = AssetDatabase.LoadAssetAtPath<Texture2D>(IconRelative.Replace("/", "\\"));
        ...
        {
            if (!string.IsNullOrEmpty(IconRelative)) return AssetDatabase.LoadAssetAtPath<Texture2D>(IconRelative.Replace("/", "\\"));

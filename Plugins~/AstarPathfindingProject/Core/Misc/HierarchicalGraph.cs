...
namespace Pathfinding
{
    ...
    public class HierarchicalGraph
    {
        ...
        private List<GraphNode>[] children = Array.Empty<List<GraphNode>>();
        private List<int>[] connections = Array.Empty<List<int>>();
        private int[] areas = Array.Empty<int>();
        private byte[] dirty = Array.Empty<byte>();
            ...
            connectionCallback = (GraphNode neighbour) =>
            {
                ...
                if (hIndex == 0)
                {
                    if (currentChildren.Count < MaxChildrenPerNode &&
                        neighbour.Walkable /* && (((GridNode)currentChildren[0]).XCoordinateInGrid/Tiling == ((GridNode)neighbour).XCoordinateInGrid/Tiling) && (((GridNode)currentChildren[0]).ZCoordinateInGrid/Tiling == ((GridNode)neighbour).ZCoordinateInGrid/Tiling)*/
                       )
                    {
        ...
        private void RemoveHierarchicalNode(int hierarchicalNode, bool removeAdjacentSmallNodes)
        {
            ...
            foreach (var item in nodeChildren) AddDirtyNode(item);
        ...
        public void OnDrawGizmos(Pathfinding.Util.RetainedGizmos gizmos)
        {
            ...
            if (!gizmos.Draw(hasher))
            {
                ...
                var centers = new List<Vector3>(areas.Length);
                for (var i = 0; i < areas.Length; i++)
                {
                    ...
                    if (childs.Count > 0)
                    {
                        center = childs.Aggregate(center, (current, item) => current + item.position);
                        center /= childs.Count;

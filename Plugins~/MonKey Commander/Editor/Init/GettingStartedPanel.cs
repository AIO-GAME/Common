...
public class GettingStartedPanel : EditorWindow
{
    ...
    private static void ShowPanelFirstTime()
    {
        ...
        MonKeySettingsEditor.Instance.MaxSortedSelectionSize = 200;
    ...
    {
        var panel = GetWindow<GettingStartedPanel>();
        InitGraphics(panel);
    ...
    private static void InitGraphics(GettingStartedPanel panel)
    {
        ...
        panel.buttonStyle = new GUIStyle()
        {
            ...
            hover =
            {
                background = MonkeyStyle.ColorTexture(1, 1,
    ...
    private const string mainTitleText = "A Wild MonKey Appeared!";

    private const string thankYouText =
        "Thank you for choosing us to assist you!" +
        " \n This should help you maximize your productivity:";

    private const string rateUsTitle = "RATE US!";

    private readonly string rateUs = "Liking MonKey? If you could rate us on the asset store, it would be great :)".Bold();

    private const string gettingStartedTitle = "Getting Started";

    private const string gettingStartedText = "Learn how to be efficient with MonKey, and how to understand everything";
    private const string userManualText = "CHECK THE MANUAL!";
    private const string top10Commands = "OUR TOP 10 COMMANDS";

    private const string commandsInDepthTitle = "Commands In Depth";
    private const string commandsInDepthText = "Want to know more advanced things?";
    private const string checkCommandsText = "FULL LIST OF COMMANDS";
    private const string writeYourOwnText = "HOW TO WRITE COMMANDS?";
    private const string needHelpTitle = "Need Help?";
    private const string needHelpText = "Something isn't right, you have a problem or you want to give us feedback?";
    private const string chatOnDiscordText = "CHAT ON DISCORD!";
    private const string supportText = "SUPPORT";
    ...
    {
        if (monKeyBannerStyle == null) InitGraphics(this);
    ...
    private void RateUseSection()
    {
        ...
        GUILayout.Label("", buttonTop);
        var button = GUILayout.Button(rateUsTitle.Colored(titleColor).Bold(), newVersionButtonStyle);
        GUILayout.Label("", buttonBottom);
        GUILayout.EndVertical();
        GUILayout.EndVertical();

        if (button) Application.OpenURL("https://assetstore.unity.com/packages/tools/utilities/monkey-commander-productivity-booster-119938");
    ...
    private void GettingStartedSection()
    {
        ...
        GUILayout.BeginVertical(newVersionButtonBoxStyle);
        GUILayout.Label("", buttonTop);
        var userManual = GUILayout.Button(userManualText.Colored(titleColor).Bold(), buttonStyle);
        if (userManual) Application.OpenURL("https://sites.google.com/view/monkey-user-guide/home");
        GUILayout.Label("", buttonBottom);
        GUILayout.EndVertical();
        ...
        GUILayout.BeginVertical(newVersionButtonBoxStyle);
        GUILayout.Label("", buttonTop);
        var top10 = GUILayout.Button(top10Commands.Colored(titleColor).Bold(), buttonStyle);
        GUILayout.Label("", buttonBottom);
        GUILayout.EndVertical();

        if (top10) Application.OpenURL("https://sites.google.com/view/monkey-user-guide/getting-started");
        GUILayout.FlexibleSpace();
    ...
    private void InDepthSection()
    {
        ...
        var commandlink = GUILayout.Button(checkCommandsText.Colored(titleColor).Bold(), buttonStyle);

        GUILayout.Label("", buttonBottom);
        GUILayout.EndVertical();

        if (commandlink) Application.OpenURL("https://sites.google.com/view/monkey-user-guide/command-list");

        GUILayout.FlexibleSpace();

        GUILayout.BeginVertical(newVersionButtonBoxStyle);
        GUILayout.Label("", buttonTop);
        var writeYourOwn = GUILayout.Button(writeYourOwnText.Colored(titleColor).Bold(), buttonStyle);
        GUILayout.Label("", buttonBottom);
        GUILayout.EndVertical();

        if (writeYourOwn) Application.OpenURL("https://sites.google.com/view/monkey-user-guide/command-creation");
        GUILayout.FlexibleSpace();
    ...
    private void NeedHelpSection()
    {
        ...
        GUILayout.BeginVertical(newVersionButtonBoxStyle);
        GUILayout.Label("", buttonTop);
        var discord = GUILayout.Button(chatOnDiscordText.Colored(titleColor).Bold()
            , buttonStyle);
        GUILayout.Label("", buttonBottom);
        GUILayout.EndVertical();

        if (discord) Application.OpenURL("https://discordapp.com/invite/wRzsqxn");

        GUILayout.FlexibleSpace();

        GUILayout.BeginVertical(newVersionButtonBoxStyle);
        GUILayout.Label("", buttonTop);
        var support = GUILayout.Button(supportText.Colored(titleColor).Bold(), buttonStyle);
        if (support) Application.OpenURL("https://sites.google.com/view/monkey-commander/support");
        GUILayout.Label("", buttonBottom);
        GUILayout.EndVertical();

...
namespace Paroxe.PdfRenderer
{
    ...
    public class PDFDocument : IPDFDocumentInternal, IDisposable
    {
        ...
        private Dictionary<int, List<WeakReference>> m_LoadedPages;
	        ...
	        if (!m_Disposed)
	        {
                ...
                lock (PDFLibrary.nativeLock)
                {
                    if (m_NativePointer != IntPtr.Zero)
                    {
	                    ...
	                    {
		                    foreach (List<WeakReference> references in m_LoadedPages.Values)
		                    {
			                    foreach (WeakReference reference in references)
			                    {
				                   ...
				                   PDFPage page = (PDFPage)reference.Target;
        ...
        void IPDFDocumentInternal.OnPageClose(PDFPage page)
        {
	        if (m_LoadedPages != null && !m_Disposing)
	        {
		        ...
		        if (m_LoadedPages.TryGetValue(page.PageIndex, out references))
			        references.RemoveAll(r => !r.IsAlive || ReferenceEquals(page, (PDFPage)r.Target));
	        }
        }
        ...
        public PDFPage GetPage(int index)
        {
            ...
            if (m_LoadedPages == null)
	            m_LoadedPages = new Dictionary<int, List<WeakReference>>();

            List<WeakReference> references;
            if (!m_LoadedPages.TryGetValue(index, out references))
	            m_LoadedPages.Add(index, references = new List<WeakReference>());

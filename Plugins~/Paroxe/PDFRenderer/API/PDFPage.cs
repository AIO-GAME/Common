...
namespace Paroxe.PdfRenderer
{
    ...
    public class PDFPage : IIPDFPageInternal, IDisposable, IEquatable<PDFPage>
    {
        ...
        private List<WeakReference> m_LoadedTextPages;
#endif
                ...
                if (m_NativePointer != IntPtr.Zero)
                {
                    ...
                    lock (PDFLibrary.nativeLock)
                    {
                        ...
                        if (s_InstanceMap[m_NativePointer] == 0)
                        {
                            ...
                            {
	                            foreach (WeakReference reference in m_LoadedTextPages)
	                            {
		                            if (!reference.IsAlive)
                                        continue;
		                            
		                            PDFTextPage textPage = (PDFTextPage)reference.Target;
        ...
        void IIPDFPageInternal.OnTextPageClose(PDFTextPage textPage)
        {
	        if (m_LoadedTextPages != null && !m_Disposing)
		        m_LoadedTextPages.RemoveAll(r => !r.IsAlive || ReferenceEquals(textPage, (PDFTextPage)r.Target));
        }
#endif
        ...
        public PDFTextPage GetTextPage()
        {
            ...
            if (m_LoadedTextPages == null)
	            m_LoadedTextPages = new List<WeakReference>();

            m_LoadedTextPages.Add(new WeakReference(textPage));

...
namespace Shader.Control.Editor
{
    ...
    public partial class SCWindow
    {
        ...
        private enum KeywordScopeFilter
        {
            Any = 0,
            GlobalKeywords = 1,
            LocalKeywords = 2
        ...
        private SortType sortType = SortType.VariantsCount;
        private GUIStyle blackStyle, commentStyle, disabledStyle, foldoutBold, foldoutNormal, foldoutDim, foldoutRTF;
        private GUIStyle titleStyle, redButtonStyle, labelDim, labelNormal;
        private Vector2 scrollViewPosProject;
        private bool firstTime;
        private GUIContent matIcon, shaderIcon;
        private bool scanAllShaders;
        private string keywordFilter;
        private KeywordScopeFilter keywordScopeFilter;
        private PragmaTypeFilter pragmaTypeFilter;
        private ModifiedStatus modifiedStatus;
        private bool showShadersNotUsedInBuild;
        private bool showKeywordsNotUsedInBuild;
        private bool hideReadOnlyShaders;
        private string projectShaderNameFilter;
        private string projectShaderPathFilter;

        private void DrawProjectGUI()
        {
            GUILayout.Box(
                new GUIContent(
                    "List of shader files in your project and modify their keywords.\nOnly shaders with source code or referenced by materials are included in this tab.\nUse the <b>Build View</b> tab for a list of shaders and keywords used in your build, including hidden/internal Unity shaders."),
            ...
            if (shaders != null && shaders.Count > 0 && GUILayout.Button(
                    new GUIContent("Clean All Materials",
                        "Removes all disabled keywords in all materials.  This option is provided to ensure no existing materials are referencing a disabled shader keyword.\n\nTo disable keywords, first expand any shader from the list and uncheck the unwanted keywords (press 'Save' to modify the shader file and to clean any existing material that uses that specific shader)."),
                    GUILayout.Width(130)))
            {
                CleanAllMaterials();
                GUIUtility.ExitGUI();
            ...
            var totalGlobalKeywordsTxt = totalGlobalKeywords != totalKeywords ? " (" + totalGlobalKeywords + " global) " : " ";
            string statsText;
            if (totalKeywords == totalUsedKeywords || totalKeywords == 0)
            {
                statsText = "Shaders Found: " + totalShaderCount + "  Shaders Using Keywords: " + shaders.Count + "\nKeywords: " + totalKeywords +
                            totalGlobalKeywordsTxt + " Variants: " + totalVariants;
            }
            else
            {
                var keywordsPerc = totalUsedKeywords * 100 / totalKeywords;
                var variantsPerc = totalBuildVariants * 100 / totalVariants;
                statsText = "Shaders Found: " + totalShaderCount + "  Shaders Using Keywords: " + shaders.Count + "\nUsed Keywords: " + totalUsedKeywords + " of " +
                            totalKeywords + " (" + keywordsPerc + "%) " + totalGlobalKeywordsTxt + " Actual Variants: " + totalBuildVariants + " of " +
                            totalVariants + " (" + variantsPerc + "%)";
            }
            ...
            var shaderCount = shaders.Count;
            if (shaderCount > 0)
            {
                EditorGUILayout.BeginHorizontal();
                EditorGUILayout.LabelField("Sort By", GUILayout.Width(100));
                var prevSortType = sortType;
                ...
                if (sortType != SortType.Material)
                {
                        ...
                        if (showShadersNotUsedInBuild)
                        {
                            if (totalBuildShaders == 0)
                            {
                                EditorGUILayout.HelpBox(
                                    "No build data yet. Perform a build to detect which shaders are being included in your build. Use 'Quick Build' from the Build View tab to make a fast build.",
                                    MessageType.Error);
                            }
                        }
                    }
                    ...
                    if (keywordScopeFilter != KeywordScopeFilter.GlobalKeywords || pragmaTypeFilter != PragmaTypeFilter.ShaderFeature)
                    {
                        ...
                        {
                            if (GUILayout.Button(new GUIContent("Click to show " + totalGlobalShaderFeaturesNonReadonly + " global keyword(s) found of type 'shader_feature'",
                                    "Lists all shaders using global keywords that can be converted to local safely.")))
                            {
                    ...
                    else
                    {
                        ...
                        {
                            if (GUILayout.Button(new GUIContent("Convert " + totalGlobalShaderFeaturesNonReadonly + " keyword(s) to local",
                                    "Converts all global keywords of type Shader_Feature to local keywords, reducing the total keyword usage count.\n A local keyword does not count towards the limit of 256 total keywords in project.")))
                            {
                                if (EditorUtility.DisplayDialog("Convert " + totalGlobalShaderFeaturesNonReadonly + " global keyword(s) to local",
                                        "The keywords will be converted to local. This means these keywords won't count toward the maximum global keyword limit (256). Continue?", "Ok", "Cancel"))
                                {
                                    ConvertToLocalAll();
            ...
            else if (sortType == SortType.Keyword)
            {
                if (keywordView != null)
                {
                    ...
                    for (int s = 0; s < kvCount; s++)
                    {
                        ...
                        if (keywordScopeFilter != KeywordScopeFilter.Any && ((keywordScopeFilter == KeywordScopeFilter.GlobalKeywords && !keyword.isGlobal) ||
                                                                             (keywordScopeFilter == KeywordScopeFilter.LocalKeywords && keyword.isGlobal))) continue;
                        if (pragmaTypeFilter != PragmaTypeFilter.Any && ((pragmaTypeFilter == PragmaTypeFilter.MultiCompile && !keyword.isMultiCompile) ||
                                                                         (pragmaTypeFilter == PragmaTypeFilter.ShaderFeature && keyword.isMultiCompile))) continue;
                        ...
                        {
                            var included = false;
                            for (var index = 0; index < totalBuildShaders; index++)
                            {
                                var sbi = shadersBuildInfo.shaders[index];
                                var count = sbi.keywords?.Count ?? 0;
                                for (var kb = 0; kb < count; kb++)
                                {
                        ...
                        var kvShadersCount = kv.shaders.Count;
                        ...
                        if (keyword.canBeConvertedToLocal)
                        {
                            ...
                            {
                                if (EditorUtility.DisplayDialog("Convert global keyword to local",
                                        "Keyword " + keyword.name + " will be converted to local. This means the keyword won't count toward the maximum global keyword limit (256). Continue?", "Ok",
                                        "Cancel"))
                                {
                        ...
                        if (kv.foldout)
                        {
                            for (int m = 0; m < kvShadersCount; m++)
                            {
                                ...
                                {
                                    var theShader = AssetDatabase.LoadAssetAtPath<UnityEngine.Shader>(shader.path);
                ...
                if (bkvCount > 0)
                {
                    ...
                    for (int s = 0; s < bkvCount; s++)
                    {
                        ...
                        EditorGUILayout.BeginHorizontal();
                        keywordViewExtra[s].foldout = EditorGUILayout.Foldout(bkv.foldout,
                            new GUIContent("Keyword " + (rowNumber++) + ": <b>" + keywordName + "</b> found in " + bkv.shaders.Count + " shader(s)"), foldoutRTF);
                        EditorGUILayout.EndHorizontal();
                        if (bkv.foldout)
                        {
            ...
            else
            {
                ...
                for (int s = 0; s < shaderCount; s++)
                {
                    ...
                    if (filterByKeywordName || keywordScopeFilter != KeywordScopeFilter.Any || pragmaTypeFilter != PragmaTypeFilter.Any)
                    {
                        ...
                        for (int w = 0; w < kwCount; w++)
                        {
                            ...
                            if (keywordScopeFilter != KeywordScopeFilter.Any && ((keywordScopeFilter == KeywordScopeFilter.GlobalKeywords && !keyword.isGlobal) ||
                                                                                 (keywordScopeFilter == KeywordScopeFilter.LocalKeywords && keyword.isGlobal)))
                            {
                                found = false;
                            }

                            if (pragmaTypeFilter != PragmaTypeFilter.Any && ((pragmaTypeFilter == PragmaTypeFilter.MultiCompile && !keyword.isMultiCompile) ||
                                                                             (pragmaTypeFilter == PragmaTypeFilter.ShaderFeature && keyword.isMultiCompile)))
                            {
                                found = false;
                            }
                    ...
                    if (showShadersNotUsedInBuild && totalBuildShaders > 0)
                    {
                        ...
                        for (var index = 0; index < totalBuildShaders; index++)
                        {
                            if (shadersBuildInfo.shaders[index].name.Equals(shader.fullName))
                            {
                                included = true;
                                break;
                            }
                    ...
                    if (shader.hasSource)
                    {
                        shader.foldout = EditorGUILayout.Foldout(shader.foldout,
                            new GUIContent(shaderName + " (" + shader.keywords.Count + " keywords, " + shader.keywordEnabledCount + " used, " + shader.actualBuildVariantCount + " variants)"),
                            shader.editedByShaderControl ? foldoutBold : foldoutNormal);
                    }
                    else
                    {
                    ...
                    if (shader.foldout)
                    {
                        ...
                        if (shader.hasSource)
                        {
                            ...
                            if (!shader.isShaderGraph)
                            {
                                ...
                                if (GUILayout.Button(
                                        new GUIContent("Save",
                                            "Saves any keyword change to the shader file (disable/enable the keywords just clicking on the toggle next to the keywords shown below). A backup is created in the same folder."),
                        ...
                        if (showShadersNotUsedInBuild)
                        {
                            ...
                            if (GUILayout.Button(new GUIContent("Delete", "Deletes shader file."), redButtonStyle))
                            {
                                ...
                                else if (EditorUtility.DisplayDialog("Delete Shader",
                                             "Are you sure you want to delete the shader " + shader.name + " at " + shader.path +
                                             "?\n\nThis operation is not reversible - make sure you have a backup or way to import the shader again if you need it later.", "Yes - Delete It",
                                             "Cancel"))
                                {
                                    DeleteShader(shader);
                                    GUIUtility.ExitGUI();
                        ...
                        {
                            if (GUILayout.Button(new GUIContent(shader.showMaterials ? "Hide Materials" : "List Materials", "Show or hide the materials that use these keywords."),
                                    EditorStyles.miniButton))
                            {
                            ...
                            if (shader.showMaterials)
                            {
                                ...
                                {
                                    var matCount = shader.materials.Count;
                                    var allMaterials = new List<Material>();
                                    for (var m = 0; m < matCount; m++)
                                    {
                                        var material = shader.materials[m];
                                ...
                                if (GUILayout.Button("Select Materials In Scene", EditorStyles.miniButton))
                                {
                                    ...
                                    var rr = FindObjectsOfType<Renderer>();
                                    foreach (var r in rr)
                                    {
                                        var go = r.gameObject;
                                        if (go == null) continue;
                                        ...
                                        for (var m = 0; m < matCount; m++)
                                        {
                                            var mat = shader.materials[m].unityMaterial;
                        ...
                        {
                            if (keyword.isUnderscoreKeyword) continue;

                            EditorGUILayout.BeginHorizontal();
                            EditorGUILayout.LabelField("", GUILayout.Width(15));
                            if (shader.hasSource && !shader.isShaderGraph)
                            {
                                var prevState = keyword.enabled;
                            ...
                            if (keyword.enabled)
                            {
                                if (!shader.hasSource && GUILayout.Button(new GUIContent("Prune Keyword", "Removes the keyword from all materials that reference it."), EditorStyles.miniButton,
                                        GUILayout.Width(110)))
                                {
                                    if (EditorUtility.DisplayDialog("Prune Keyword",
                                            "This option will disable the keyword " + keyword.name + " in all materials that use " + shader.name + " shader.\nDo you want to continue?", "Ok",
                                            "Cancel"))
                                    {
                                ...
                                if (keyword.canBeConvertedToLocal)
                                {
                                    ...
                                    {
                                        if (EditorUtility.DisplayDialog("Convert global keyword to local",
                                                "Keyword " + keyword.name + " will be converted to local. This means the keyword won't count toward the maximum global keyword limit (256). Continue?",
                                                "Ok", "Cancel"))
                                        {
                                            ConvertToLocalStarted();
                                            ConvertToLocal(keyword);
                            ...
                            if (shader.showMaterials)
                            {
                                ...
                                for (int m = 0; m < matCount; m++)
                                {
                                    ...
                                    if (material.ContainsKeyword(keyword.name))
                                    {
                                        ...
                                        {
                                            var theMaterial = AssetDatabase.LoadAssetAtPath<Material>(material.path);
                ...
                if (shadersListedCount == 0)
                {
                    ...
                    {
                        EditorGUILayout.HelpBox(
                            "No shader matching '" + projectShaderNameFilter + "' name in '" + projectShaderPathFilter +
                            "' path found. Either the shader doesn't exist in the path as source file or the shader does not use explicit keywords.", MessageType.Info);
                    }
                    else if (usesShaderNameFilter)
                    {
                        EditorGUILayout.HelpBox(
                            "No shader matching '" + projectShaderNameFilter +
                            "' name found. Either the shader doesn't exist in the project as source file or the shader does not use explicit keywords.", MessageType.Info);
                    }
                    else
                    {
                        EditorGUILayout.HelpBox(
                            "No shader matching criteria found in '" + projectShaderPathFilter +
                            "' path found. Either the shader doesn't exist in the path as source file or the shader does not use explicit keywords.", MessageType.Info);
                    }
                }
            }

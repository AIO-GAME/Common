...
namespace Shader.Control.Editor
{
    ...
    public partial class SCWindow
    {
        ...
        private const string EDITOR_PREFIX = "ShaderControl_";

        private ViewMode viewMode;
        private string[] viewModeTexts = { "Build View", "Project View" };
        ...
        {
            // Setup styles
            var backColor = EditorGUIUtility.isProSkin ? new Color(0.18f, 0.18f, 0.18f) : new Color(0.7f, 0.7f, 0.7f);
            var _blackTexture = MakeTex(4, 4, backColor);
        ...
        private void OnGUI()
        {
            ...
            if (matIcon == null)
            {
                matIcon = EditorGUIUtility.IconContent("PreMatSphere") ?? new GUIContent();
            }

            if (shaderIcon == null)
            {
                shaderIcon = EditorGUIUtility.IconContent("Shader Icon");
                if (shaderIcon == null) matIcon = new GUIContent();
            }
        ...
        private void ShowHelpWindowFirstTime()
        {
            if (firstTime)
            {

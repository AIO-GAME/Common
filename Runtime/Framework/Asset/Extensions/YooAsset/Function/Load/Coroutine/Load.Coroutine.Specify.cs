...
namespace AIO.UEngine
{
    ...
    internal partial class YAssetSystem
    {
        ...
        public static IEnumerator InstGameObjectCO(string packagename, string location, Transform parent, Action<GameObject> cb)
        {
        ...
        public static IEnumerator LoadSubAssetsCO<TObject>(string packagename, string location, Action<TObject[]> cb) where TObject : Object
        {
        ...
        /// <param name="location">资源的定位地址</param>
        /// <param name="cb">回调</param>
        public static IEnumerator LoadAssetCO<TObject>(string packagename, string location, Action<TObject> cb) where TObject : Object
        {
            var operation = GetHandle<AssetOperationHandle>(location);
            if (operation is null)
            {
            ...
            int priority = 100)
        {
            ...
            if (operation is null)
            {
                YAssetPackage package = null;
                yield return GetAutoPakcageCO(packagename, location, ya => package = ya);
                if (package is null) throw new Exception(string.Format("场景配置 异常错误:{0} {1} {2}", packagename, location, sceneMode));
                ...
                if (!check) throw new Exception(string.Format("加载场景 资源异常:{0} {1} {2}", packagename, location, sceneMode));
                AddHandle(location, operation);
            }

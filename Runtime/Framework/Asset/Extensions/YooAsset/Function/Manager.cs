    ...
    internal partial class YAssetSystem
    {
        ...
        public static IEnumerator LoadCO()
        {
            ...
            foreach (var item in Dic.Values)
            {
                ...
                else Print.LogFormat("{0} -> {1} -> {2}", nameof(LoadTask), item.Config, operation.Error);
            }
...
#endif
                else Print.ErrorFormat("{0} -> {1} -> {2}", nameof(LoadTask), item.Config);
            }

...
{
    /// <summary>
    /// 文件夹对象脚本, 用于在层级视图中组织对象, 在构建时或运行时将其子对象分离到场景根目录
    /// </summary>
    ...
    public class HierarchyFolderObject : MonoBehaviour
    {
        ...
        {
            [InspectorName("At Build Time")]
            AtBuildTime = 0,

            [InspectorName("On Awake")]
            OnAwake = 1,

            [InspectorName("Via Script")]
            ViaScript = 2
        };
        ...
        public enum ActiveState
        {
            ...
            [InspectorName("Inherit From Active In Hierarchy")]
        ...
        public void Flatten()
        {
            ...
            if (GroupSize <= 1) tr.DetachChildren();
            else
            {

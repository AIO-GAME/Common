...
namespace AIO.ICSharpCode.SharpZipLib.Zip
{
	...
	{
		internal delegate void KeysRequiredEventHandler(object sender, KeysRequiredEventArgs e);
		...
		private Stream GetOutputStream(ZipEntry entry)
		{
			...
			case CompressionMethod.Deflated:
				return new DeflaterOutputStream(stream, new Deflater(9, true))
				{
					IsStreamOwner = false
				};
			default:
		...
		private void CopyEntryDirect(ZipFile workFile, ZipUpdate update, ref long destinationPosition)
		{
			...
			{
				CopyEntryDataDirect(update, baseStream_, false, ref destinationPosition, ref num2);
			}

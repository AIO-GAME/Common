...
namespace AIO.PY4N
{
    ...
    public class Pinyin4Net
    {
        ...
        {
            if (!PinyinUtil.IsChinese(hanzi)) // 不是汉字
                throw new UnsupportedUnicodeException("不支持的字符: " + hanzi);

            var pinyin = PinyinDB.Instance.GetPinyin(hanzi);
            return format == PinyinFormat.None ? pinyin : pinyin.Select(item => PinyinUtil.Format(item, format)).ToArray();
        }
        ...
        public static List<PinyinItem> GetPinyinArray(string text, PinyinFormat format)
        {
            ...
            if (string.IsNullOrEmpty(text)) return pinyin;
        ...
        public static string GetPinyin(string text, PinyinFormat format, bool caseSpread, Func<string[], char, string, string> pinyinHandler)
        {
            ...
            foreach (var item in text)
            {
                ...
                pinyin.Append(pinyinHandler == null ? pinyinTemp[0] : pinyinHandler.Invoke(pinyinTemp, item, text));
            }
        ...
        public static string GetPinyin(string text, PinyinFormat format) { return GetPinyin(text, format, false, false, false); }
        ...
        /// <returns></returns>
        public static string[] GetHanzi(string pinyin, bool matchAll) { return PinyinDB.Instance.GetHanzi(pinyin.ToLower(), matchAll).Select(item => item.ToString()).ToArray(); }
        ...
        public static void UpdateMap(Dictionary<char, string[]> data, bool replace = false) { PinyinDB.Instance.Update(data, replace); }

...
namespace AIO
{
    partial class ExtendSort
    {
        ...
        ///     快速排序
        /// </summary>
        private static IList<T> SortQuick<T>(IList<T> array, in int index, in int count, in Func<T, T, int> Comparer)
        {
            if (array.Count >= 2) Quick(array, index, count - 1, Comparer);
            return array;
        }

        internal static void Quick<T>(in IList<T> a, in int l, in int r, in Func<T, T, int> Comparer)
        {
            if (l >= r) return;
            var i = l;
            var j = r;

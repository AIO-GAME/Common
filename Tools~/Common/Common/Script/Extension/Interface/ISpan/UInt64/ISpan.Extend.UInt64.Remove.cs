...
namespace AIO
{
    partial class ExtendISpan
    {
        ...
        /// </remarks>
        public static ulong[] MRemoveAll(this ulong[] arrays, int value)
        {
            ...
            for (var i = 0; i < len; i++)
            {
                if (arrays[i].CompareTo(value) == 0) index++;
                else if (index > 0)
                {
        ...
        /// 不重新分配内存 source.<see cref="MRemoveAll{T}"/>
        /// </remarks>
        public static ulong[] MRemoveAll(this ulong[] arrays, long value)
        {
            if (arrays is null) throw new ArgumentNullException(nameof(arrays));

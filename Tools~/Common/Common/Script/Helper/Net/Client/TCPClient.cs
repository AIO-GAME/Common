...
namespace AIO.Net
{
    ...
    public class TCPClient : IDisposable
    {
        ...
        public virtual long Send(ICollection<char> text) => Send(Encoding.UTF8.GetBytes(text.ToArray()));
        ...
        public virtual bool SendAsync(ICollection<byte> buffer) => SendAsync(buffer, 0, buffer.Count);
        ...
        /// <param name="size">Buffer size</param>
        /// <returns>'true' if the data was successfully sent, 'false' if the client is not connected</returns>
        public virtual bool SendAsync(ICollection<byte> buffer, long offset, long size)
        {
            if (!IsConnected)
                return false;
            ...
            lock (_sendLock)
            {
                ...
                if (((_sendBufferMain.Size + buffer.Count) > OptionSendBufferLimit) && (OptionSendBufferLimit > 0))
                {
        ...
        public virtual bool SendAsync(IEnumerable<char> text) => SendAsync(Encoding.UTF8.GetBytes(text.ToArray()));

...
namespace AIO
{
    ...
    public class QRCodeGenerator
    {
        ...
        public QRCodeData CreateQrCode(string plainText, ECCLevel eccLevel, bool forceUtf8 = false, bool utf8BOM = false, EciMode eciMode = EciMode.Default, int requestedVersion = -1) { return GenerateQrCode(plainText, eccLevel, forceUtf8, utf8BOM, eciMode, requestedVersion); }
        ...
        /// <param name="eccLevel"> The level of error correction data</param>
        public QRCodeData CreateQrCode(byte[] binaryData, ECCLevel eccLevel) { return GenerateQrCode(binaryData, eccLevel); }
        ...
        private static class ModulePlacer
        {
            ...
            public static int MaskCode(ref QRCodeData qrCode, int version, ref List<Rectangle> blockedModules, ECCLevel eccLevel)
            {
                ...
                if (selectedPattern == null) // 不应该发生：表示某件事情是不应该发生的，是异常或意外的情况
                    selectedPattern = 1;
                for (var x = 0; x < size; x++)
                {
                    for (var y = 0; y < x; y++)
            ...
            private static bool Intersects(Rectangle r1, Rectangle r2) { return r2.X < r1.X + r1.Width && r1.X < r2.X + r2.Width && r2.Y < r1.Y + r1.Height && r1.Y < r2.Y + r2.Height; }
            ...
            {
                public static bool Pattern1(int x, int y) { return (x + y) % 2 == 0; }

                public static bool Pattern2(int x, int y) { return y % 2 == 0; }

                public static bool Pattern3(int x, int y) { return x % 3 == 0; }

                public static bool Pattern4(int x, int y) { return (x + y) % 3 == 0; }

                public static bool Pattern5(int x, int y) { return ((int)(Math.Floor(y / 2d) + Math.Floor(x / 3d)) % 2) == 0; }

                public static bool Pattern6(int x, int y) { return ((x * y) % 2) + ((x * y) % 3) == 0; }

                public static bool Pattern7(int x, int y) { return (((x * y) % 2) + ((x * y) % 3)) % 2 == 0; }

                public static bool Pattern8(int x, int y) { return (((x + y) % 2) + ((x * y) % 3)) % 2 == 0; }

                public static int Score(ref QRCodeData qrCode)
                {
                    int score1 = 0, score2 = 0, score3 = 0, score4 = 0;
                    var size   = qrCode.ModuleMatrix.Count;
                    for (var y = 0; y < size; y++) //Penalty 1
                    {
                        var modInRow      = 0;
                        var modInColumn   = 0;
        ...
        private static int GetVersion(int length, EncodingMode encMode, ECCLevel eccLevel)
        {
            ...
            var enumerable = fittingVersions.ToArray();
            if (enumerable.Any()) return enumerable.Min(x => x.version);
        ...
        private static bool IsInRange(char c, char min, char max) { return (uint)(c - min) <= (uint)(max - min); }
        ...
        private static List<int> BinaryStringListToDecList(List<string> binaryStringList) { return binaryStringList.Select(binaryString => BinToDec(binaryString)).ToList(); }

        private static int BinToDec(string binStr) { return Convert.ToInt32(binStr, 2); }

        private static string DecToBin(int decNum) { return Convert.ToString(decNum, 2); }
        ...
        private static int GetDataLength(EncodingMode encoding, string plainText, string codedText, bool forceUtf8) { return forceUtf8 || IsUtf8(encoding, plainText, forceUtf8) ? (codedText.Length / 8) : plainText.Length; }

        private static bool IsUtf8(EncodingMode encoding, string plainText, bool forceUtf8) { return (encoding == EncodingMode.Byte && (!IsValidISO(plainText) || forceUtf8)); }
        ...
        private static Polynom MultiplyAlphaPolynoms(Polynom polynomBase, Polynom polynomMultiplier)
        {
            ...
            {
                var coefficient = resultPolynom.PolyItems.Where(x => x.Exponent == exponent).
                                                Aggregate(0, (current, polynomOld)
        ...
        private static int GetIntValFromAlphaExp(int exp) { return galoisField.Find(alog => alog.ExponentAlpha == exp).IntegerValue; }

        private static int GetAlphaExpFromIntVal(int intVal) { return galoisField.Find(alog => alog.IntegerValue == intVal).ExponentAlpha; }
        ...
        {
            public Polynom() { this.PolyItems = new List<PolynomItem>(); }

namespace AIO
{
    public class OrHandler : BinaryOperatorHandler
    {
        public OrHandler() : base("Or", "Or", "|", "op_BitwiseOr")
        {
            ...
            Handle<byte, sbyte>((a, b) => a | (byte)b);
            Handle<byte, short>((a, b) => a | (ushort)b);
            ...
            Handle<sbyte, byte>((a, b) => (byte)a | b);
            Handle<sbyte, sbyte>((a, b) => a | b);
            Handle<sbyte, short>((a, b) => (byte)a | (ushort)b);
            Handle<sbyte, ushort>((a, b) => (byte)a | b);
            Handle<sbyte, int>((a, b) => (byte)a | b);
            Handle<sbyte, uint>((a, b) => (byte)a | b);
            Handle<sbyte, long>((a, b) => (byte)a | b);
            //Handle<sbyte, ulong>((a, b) => a | b);

            Handle<short, byte>((a, b) => (ushort)a | b);
            Handle<short, sbyte>((a, b) => (ushort)a | (byte)b);
            Handle<short, short>((a, b) => a | b);
            Handle<short, ushort>((a, b) => (ushort)a | b);
            Handle<short, int>((a, b) => (ushort)a | b);
            Handle<short, uint>((a, b) => (ushort)a | b);
            Handle<short, long>((a, b) => (ushort)a | b);
            //Handle<short, ulong>((a, b) => a | b);

            Handle<ushort, byte>((a, b) => a | b);
            Handle<ushort, sbyte>((a, b) => a | (byte)b);
            Handle<ushort, short>((a, b) => a | (ushort)b);
            ...
            Handle<int, sbyte>((a, b) => a | (byte)b);
            Handle<int, short>((a, b) => a | (ushort)b);
            Handle<int, ushort>((a, b) => a | b);
            Handle<int, int>((a, b) => a | b);
            Handle<int, uint>((a, b) => (uint)a | b);
            Handle<int, long>((a, b) => (uint)a | b);
            //Handle<int, ulong>((a, b) => a | b);

            Handle<uint, byte>((a, b) => a | b);
            Handle<uint, sbyte>((a, b) => a | (byte)b);
            Handle<uint, short>((a, b) => a | (ushort)b);
            Handle<uint, ushort>((a, b) => a | b);
            Handle<uint, int>((a, b) => a | (uint)b);
            Handle<uint, uint>((a, b) => a | b);
            Handle<uint, long>((a, b) => a | b);
            Handle<uint, ulong>((a, b) => a | b);

            Handle<long, byte>((a, b) => a | b);
            Handle<long, sbyte>((a, b) => a | (byte)b);
            Handle<long, short>((a, b) => a | (ushort)b);
            Handle<long, ushort>((a, b) => a | b);
            Handle<long, int>((a, b) => a | (uint)b);

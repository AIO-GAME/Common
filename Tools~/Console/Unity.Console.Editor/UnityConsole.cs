...
public static class UnityConsoleEditor
{
    ...
    private static void EditorProxy()
    {
        ...
        if (IS_EDITOR_SWITCH_ERROR) UnityConsole.ENABLED_ERROR(); else UnityConsole.DISABLE_ERROR();
        if (IS_EDITOR_SWITCH_LOG) UnityConsole.ENABLED_LOG(); else UnityConsole.DISABLE_LOG();
    }

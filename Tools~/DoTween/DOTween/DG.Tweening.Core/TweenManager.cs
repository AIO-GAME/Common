	...
	internal static class TweenManager
	{
		...
		internal static TweenerCore<T1, T2, TPlugOptions> GetTweener<T1, T2, TPlugOptions>() where TPlugOptions : struct, IPlugOptions
		{
			...
			else if (totTweeners >= maxTweeners - 1)
			{
				...
				{
					Debugger.LogWarning("Max Tweens reached: capacity has automatically been increased from #0 to #1. Use DOTween.SetTweensCapacity to set it manually at startup".Replace("#0", string.Concat(num2.ToString(), "/", num3.ToString())).Replace("#1", string.Concat(maxTweeners.ToString(), "/", maxSequences.ToString())));
				}
			}
		...
		internal static Sequence GetSequence()
		{
			...
			if (totSequences >= maxSequences - 1)
			{
				...
				if (Debugger.logPriority >= 1)
				{
					Debugger.LogWarning("Max Tweens reached: capacity has automatically been increased from #0 to #1. Use DOTween.SetTweensCapacity to set it manually at startup".Replace("#0", string.Concat(num.ToString(), "/", num2.ToString())).Replace("#1", string.Concat(maxTweeners.ToString(), "/", maxSequences.ToString())));
				}
			}

...
namespace Borodar.RainbowFolders
{
	public class RainbowFoldersPopup : CoreDraggablePopupWindow
	{
		...
		public void ShowWithParams(Vector2 inPosition, List<string> paths, int pathIndex)
		{
			...
			Show<RainbowFoldersPopup>(inPosition2);
		}
		...
		private static void DrawPreview(ref Rect rect, ProjectRule rule)
		{
			...
			GUI.DrawTexture(image: (!rule.HasLargeIcon()) ? ProjectEditorUtility.GetDefaultFolderIcon() : (rule.HasCustomIcon() ? rule.LargeIcon : ProjectIconsStorage.GetIcons(rule.IconType).Item1), position: rect);
			rect.y += 44f;
			num3 = (rect.width = (rect.height = 16f));
			GUI.DrawTexture(image: (!rule.HasSmallIcon()) ? ProjectEditorUtility.GetDefaultFolderIcon() : (rule.HasCustomIcon() ? rule.SmallIcon : ProjectIconsStorage.GetIcons(rule.IconType).Item2), position: rect);
			rect.y += 19f;
			rect.width = 64f;

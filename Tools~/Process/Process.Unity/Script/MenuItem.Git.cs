	...
	internal static partial class PackageGen
	{
		...
		internal static void Generate()
		{
			...
			var packageInfos = AssetDatabase.FindAssets("package", new string[] { "Packages" })
				...
				.Select(x => x.First())
				.Where(x => File.Exists(Path.Combine(dataPath, x.resolvedPath, ".git")) || Directory.Exists(Path.Combine(dataPath, x.resolvedPath, ".git")))
				.ToList();
		...
		private static string GetOutPath()
		{
			...
			if (package is null || string.IsNullOrEmpty(package.resolvedPath))
				return Path.Combine(UnityEngine.Application.dataPath, "Editor/AutomaticGeneration/Git");
			return Path.Combine(package.resolvedPath, "Editor/AutomaticGeneration/Git");
		}

		public static void CreateTemplate(IEnumerable<PackageInfo> infos)
		{
			var ProjectPath = new DirectoryInfo(UnityEngine.Application.dataPath).Parent?.FullName.Replace('\\', '/')
				.Trim('\\', '/');
			if (string.IsNullOrEmpty(ProjectPath))
			{
				UnityEngine.Debug.LogError("Application.dataPath is null");
				return;
			}

namespace AIO
{
    public partial class PrCmd
    {
        ...
        public static class MkLink
        {
            ...
            /// <returns>执行器</returns>
            public static IExecutor Execute(in string target, in string source, in string args)
            {
                var messages = string.Format("{0} /D /J \"{1}\" \"{2}\"", CMD_Mklink, args, target.Replace('/', '\\'), source.Replace('/', '\\'));

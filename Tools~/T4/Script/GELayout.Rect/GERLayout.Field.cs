    ...
    public partial class GERLayoutSingleton
    {
        private static List<FunctionChunk> Field_Witch_No_Label(bool hasLabel, params string[] types)
        {
            ...
            foreach (var type in types)
            {
                ...
                if (hasLabel) paramList.Add(new[] { rect, value });
                paramList.Add(new[] { rect, label, value });
                if (hasLabel) paramList.Add(new[] { pos, size, value });
                paramList.Add(new[] { pos, size, label, value });

            ...
            foreach (var item in new[] { "string", "GUIContent" })
            {
                ...
                foreach (var displayedOption in displayedOptions)
                {
                    ...
                    foreach (var optionValue in optionValues)
                    {
                        var paramsList = new[]
                        {
                            new[] { value, displayedOption, optionValue, Options },
                            new[] { label, value, displayedOption, optionValue, Options },
                            new[] { value, displayedOption, optionValue, Style, Options },
                            new[] { label, value, displayedOption, optionValue, Style, Options }
                        };
        ...
        private static IEnumerable<FunctionChunk> GetPopupEnum_2018_3_OR_NEWER()
        {
            ...
            foreach (var item in new[] { "string", "GUIContent" })
            {
                ...
                var paramsList = new[]
                {
                    ...
                    new[] { label, value, checkEnabled, includeObsolete, Options },
                    new[] { label, value, checkEnabled, includeObsolete, Style, Options }
                };
                foreach (var param in paramsList)
                {
                    var chunk = new FunctionChunk

...
namespace AIO
{
    ...
    public static partial class SortUtils
    {
        ...
        public static IList<int> Counting(in IList<int> array)
        {
            ...
            var maxmin = array.GetMaxMinValue();
            var bucket = new int[maxmin.Item1 - maxmin.Item2 + 1]; // 初始化新组并赋值数组长度比实际个数大一
            for (var i = 0; i < bucket.Length; i++) bucket[i] = 0;
            var bias = 0 - maxmin.Item2;                           // 正填充
            for (var i = 0; i < array.Count; i++) bucket[array[i] + bias]++;
            var arrayIndex = 0; var bucketIndex = 0;               // 反填充
        ...
        public static IList<long> Counting(in IList<long> array)
        {
            if (array.Count < 2) return array;                     // 求最大最小值
            var maxmin = array.GetMaxMinValue();
            var bucket = new long[maxmin.Item1 - maxmin.Item2 + 1];// 初始化新组并赋值数组长度比实际个数大一
            for (var i = 0; i < bucket.Length; i++) bucket[i] = 0;
            var bias = 0 - maxmin.Item2;                           // 正填充
            for (var i = 0; i < array.Count; i++) bucket[array[i] + bias]++;
            var arrayIndex = 0; var bucketIndex = 0;               // 反填充
        ...
        public static IList<long> Radix(in IList<long> array)
        {
            ...
            var Max = array.GetMaxValue(); var maxDigit = 0;
            if (Max == 0) maxDigit = 1;
            else for (var temp = Max; temp != 0; temp /= 10) maxDigit++;
            for (int i = 0, mod = 10, dev = 1, pos = 0; i < maxDigit; i++, dev *= 10, mod *= 10, pos = 0)
            {
                var counter = new long[mod * 2][];
                for (int j = 0; j < array.Count; j++)
                {   // 遍历数组 将数组依照 当前排序位数中的值 进行插入
                    var bucket = (array[j] % mod / dev) + mod;
                    counter[bucket] = AutomaticArray(counter[bucket], array[j]);
                }
        ...
        public static IList<int> Radix(in IList<int> array)
        {
            ...
            else for (var temp = maxValue; temp != 0; temp /= 10) maxDigit++;
            for (int i = 0, mod = 10, dev = 1, pos = 0; i < maxDigit; i++, dev *= 10, mod *= 10, pos = 0)
            {
        ...
        public static IList<long> Bucket(in IList<long> array)
        {
            ...
            {
                var area = (array[i] - max.Item2) * array.Count / diff;   //区块
                InsertIntoLinkList(bucket[area], array[i]);
            }
        ...
        {   // 创建bucket时，在二维中增加一组标识位，其中bucket[x, 0]表示这一维所包含的数字的个数
            ...
            int bucketLen = (int)((valueMaxMin.Item1 - valueMaxMin.Item2) * coefficient) + 1;
            var bucketArray = new float[bucketLen, array.Count + 1];// 创建二维数组
            for (int i = 0; i < array.Count; i++)
            {
                var xbit = (int)((array[i] - valueMaxMin.Item2) * coefficient);
                var ybit = (int)(++bucketArray[xbit, 0]);
                bucketArray[xbit, ybit] = array[i];
            }
            for (int j = 0; j < bucketLen; j++)
            {   // 为桶里的每一行使用插入排序
                var insertion = new float[(int)bucketArray[j, 0]];                                     // 为桶里的行创建新的数组后使用插入排序
                for (int k = 0; k < insertion.Length; k++) insertion[k] = bucketArray[j, k + 1];
                Insert(insertion);                                                                     // 插入排序
                for (int k = 0; k < insertion.Length; k++) bucketArray[j, k + 1] = insertion[k];       // 把排好序的结果回写到桶里
            }
            for (int count = 0, j = 0; j < bucketLen; j++)
            {   // 将所有桶里的数据回写到原数组中
                for (int k = 1; k <= (int)bucketArray[j, 0]; k++) array[count++] = bucketArray[j, k];
            }
            return array;
        }
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static void InsertIntoLinkList<T>(in LinkedList<T> linkedList, in T num) where T : IComparable
        {
            // 链表为空时，插入到第一位  
        ...
        private static IList<T> Merge<T>(in IList<T> array, in int left, in int median, in int right, in Func<T, T, int> Comparer)
        {
            ...
            var LA = new T[LAL]; var RA = new T[RAL];
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static IList<T> Merge<T>(in IList<T> array, in int left, in int median, in int right) where T : IComparable
        {
            var LAL = median - left + 1;    //左数组长度
            var RAL = right - median;       //右数组长度
            var LA = new T[LAL]; var RA = new T[RAL];
        ...
        public static IList<T> Heap<T>(in IList<T> array) where T : IComparable
        {
            ...
            { array.Swap(0, i); HeapSort_MaxHeaping(array, 0, i); }
            return array;
        }
        ...
        /// </summary>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static void HeapSort_MaxHeaping<T>(in IList<T> array, in int index, in int size) where T : IComparable
        {
            var left = (2 * index) + 1; var right = 2 * (index + 1); var large = index;
            if (left < size && array[left].CompareTo(array[large]) > 0) large = left;
            if (right < size && array[right].CompareTo(array[large]) > 0) large = right;
            if (index != large) { array.Swap(index, large); HeapSort_MaxHeaping(array, large, size); }
        }
        ...
        public static IList<T> Heap<T>(in IList<T> array, in Func<T, T, int> Comparer)
        {
            ...
            for (int i = array.Count - 1; i > 0; i--)
            { array.Swap(0, i); HeapSort_MaxHeaping(array, 0, i, Comparer); }
            return array;
        }
        ...
        private static void HeapSort_MaxHeaping<T>(in IList<T> array, in int index, in int size, in Func<T, T, int> Comparer)
        {
            ...
            if (index != large) { array.Swap(index, large); HeapSort_MaxHeaping(array, large, size, Comparer); }
        }
        ...
        {
            if (array.Count < 2) return array;
            for (int i = 1, insertindex = i - 1; i < array.Count; i++, insertindex = i - 1)
            {
                var inservalue = array[i];
                while (insertindex >= 0 && inservalue.CompareTo(array[insertindex]) < 0)
                    array[insertindex + 1] = array[insertindex--];
                array[insertindex + 1] = inservalue;
            }
            return array;
        }
        ...
        public static IList<T> Insert<T>(in IList<T> array, in Func<T, T, int> Comparer)
        {
            ...
            {
                var inservalue = array[i];
                while (insertindex >= 0 && Comparer(inservalue, array[insertindex]) < 0)
                    array[insertindex + 1] = array[insertindex--];
                array[insertindex + 1] = inservalue;
            }
            return array;
        ...
        private static void Quick<T>(in IList<T> a, in int l, in int r) where T : IComparable
        {
            if (l < r)
            {
                ...
                a[i] = x; Quick(a, l, i - 1); Quick(a, i + 1, r);
            }
        }
        ...
        private static void Quick<T>(in IList<T> a, in int l, in int r, in Func<T, T, int> Comparer)
        {
            if (l < r)
            {
                ...
                a[i] = x; Quick(a, l, i - 1, Comparer); Quick(a, i + 1, r, Comparer);
            }
        }
        ...
        {
            if (array.Count < 2) return array;
            int L = 0, R = array.Count - 1, flag, i;
            while (L < R)
            {
                flag = 0;
                for (i = L; i < R; i++)
                    if (array[i].CompareTo(array[i + 1]) > 0)
                    { array.Swap(i, i + 1); flag = 1; }
                if (flag == 0) break;
                for (i = --R; i > L; i--)
                    if (array[i].CompareTo(array[i - 1]) < 0)
                    { array.Swap(i, i - 1); }

...
namespace AIO
{
    ...
    using ESencondUnit = TimeUnit.SencondUnit;
    using EDateTimeUnit = TimeUnit.DateTimeUnit;
    ...
    public static partial class TimeUtils
    {
        ...
        /// 昨天开始时间 单位毫秒
        /// </summary>
        public static long StartBeforeDay => GetCurrDateToDay(-2);

        /// <summary>
        /// 昨天开始时间 单位毫秒
        /// </summary>
        public static long StartYesterDay => GetCurrDateToDay(-1);

        /// <summary>
        /// 当天开始时间 单位毫秒
        /// </summary>
        public static long StartToday => GetCurrDateToDay(0);

        /// <summary>
        /// 明天开始时间 单位毫秒
        /// </summary>
        public static long StartTomorrowDay => GetCurrDateToDay(1);

        /// <summary>
        /// 后天开始时间 单位毫秒
        /// </summary>
        public static long StartAfterDay => GetCurrDateToDay(2);

        /// <summary>
        /// 获取当前时间 文字格式
        /// </summary>
        public static string GetCurrTimeStr(string fmt = "yyyy-MM-dd HH:mm:ss")
        {
        ...
        /// 获取当前时间搓
        /// </summary>
        public static long GetCurrTime(ESencondUnit Unit = ESencondUnit.MILLISCOND)
        {
        ...
        /// 获取时间搓
        /// </summary>
        public static long GetDateTime(string fmt, ESencondUnit Unit = ESencondUnit.MILLISCOND)
        {
            return Normalize(Convert.ToDateTime(fmt), Unit);
        }

        /// <summary>
        /// 格式化时间，参数：格林威治时间，格式化格式（具体见文件末尾）
        /// </summary>
        public static string Format(long time, string format = "yyyy-MM-dd 00:00:00", ESencondUnit Unit = ESencondUnit.MILLISCOND)
        {
        ...
        /// </summary>
        public static DateTime GetDateTime(long time, ESencondUnit Unit = ESencondUnit.MILLISCOND)
        {
            long Ticks;
            switch (Unit)
            {
                case ESencondUnit.SECOND:
                    Ticks = TimeUnit.NS_SECOND / 100; break;
                case ESencondUnit.MILLISCOND:
                    Ticks = TimeUnit.μS_SECOND / 100; break;
                case ESencondUnit.MICROSECOND:
                    Ticks = TimeUnit.MS_SECOND / 100; break;
                case ESencondUnit.NANOSECOND:
                    Ticks = TimeUnit.SECOND / 100; break;
                case ESencondUnit.NANOSECOND_100:
        ...
        /// </summary>
        public static TimeSpan GetTimeSpan(long time, ESencondUnit Unit = ESencondUnit.MILLISCOND)
        {
            switch (Unit)
            {
                case ESencondUnit.SECOND:
                    return new TimeSpan(time * TimeUnit.NS_SECOND / 100);
                case ESencondUnit.MILLISCOND:
                    return new TimeSpan(time * TimeUnit.μS_SECOND / 100);
                case ESencondUnit.MICROSECOND:
                    return new TimeSpan(time * TimeUnit.MS_SECOND / 100);
                case ESencondUnit.NANOSECOND:
                    return new TimeSpan(time * TimeUnit.SECOND / 100);
                case ESencondUnit.NANOSECOND_100:
        ...
        /// 获取指定单位的时间搓
        /// </summary>
        public static long Normalize(DateTime date, ESencondUnit Unit = ESencondUnit.MILLISCOND)
        {
            switch (Unit)
            {
                case ESencondUnit.SECOND: return date.Ticks / 10000000;
                case ESencondUnit.MILLISCOND: return date.Ticks / 10000;
                case ESencondUnit.MICROSECOND: return date.Ticks / 10;
                case ESencondUnit.NANOSECOND: return date.Ticks * 100;
                case ESencondUnit.NANOSECOND_100:
        ...
        /// 获取当天差距 时间信息
        /// </summary>
        public static long GetCurrDateToDay(int space = 0, ESencondUnit Unit = ESencondUnit.MILLISCOND)
        {
        ...
        public static long GetTimeStartByType(long time, EDateTimeUnit DateType, ESencondUnit Unit = ESencondUnit.MILLISCOND)
        {
        ...
        /// <param name="DateType">获取类型 年 季 月 周 日</param>
        /// <param name="Unit">时间搓 单位 纳秒 微秒 毫秒 秒</param>
        public static long GetTimeEndByType(long time, EDateTimeUnit DateType, ESencondUnit Unit = ESencondUnit.MILLISCOND)
        {
            var T = GetDateTime(time, Unit);
            switch (DateType)
            {
        ...
        /// </summary>
        public static string GetCountDown(long time, ESencondUnit Unit = ESencondUnit.MILLISCOND)
        {
        ...
        /// 获取传入时间距离当前时间的文字描述
        /// </summary>
        public static string GetPreHumanityTime(long time, ESencondUnit Unit = ESencondUnit.MILLISCOND)
        {
            if (time <= 0) return "传入值错误";

            var UNITSENCOND = TimeUnit.GetSencondUnit(Unit);
            time = (GetCurrTime(Unit) - time) / UNITSENCOND;//单位归一化

            var UNITDAY = TimeUnit.GetDayUnit(Unit);
            var day = time / UNITDAY;
            time %= UNITDAY;

            var UNITHOUR = TimeUnit.GetHourUnit(Unit);
            var hour = time / UNITHOUR;
            time %= UNITHOUR;

            var UNITMIN = TimeUnit.GetMinUnit(Unit);
            var min = time / UNITMIN;
            time %= UNITMIN;//秒
        ...
        /// </summary>
        public static bool CompareNowTime(DateTime dateTime)
        {
        ...
        /// 与当前时间比较 如果小于当前时间为Ture
        /// </summary>
        public static bool CompareNowTime(long time, ESencondUnit Unit = ESencondUnit.MILLISCOND)
        {
        ...
        public static string DateStringFromNow(DateTime dt)
        {
            ...
            else if (span.TotalDays > 7) return $"{span.TotalDays % 7}周前";
        ...
        public static bool CompareDateDay(string today, string writeDate, int n)
        {
        ...
        /// 获取两时间相差
        /// </summary>
        public static string GetDisTime(DateTime dateBegin, DateTime dateEnd, string formmat = "HH:mm:ss")
        {
            return GetDisTime(dateBegin.Ticks, dateEnd.Ticks, formmat);
        }
        ...
        {
            return (new TimeSpan(dateEnd) - new TimeSpan(dateBegin)).ToString(formmat);
        }

        /// <summary>
        /// 判断是否同日
        /// </summary>
        public static bool IsToday(long time1, long time2, ESencondUnit Unit = ESencondUnit.MILLISCOND)
        {

...
public partial class Utils
{
    ...
    public static class Conversion
    {
        ...
        private static readonly Dictionary<ConversionQuery, ConversionType> conversionTypesCache =
            new Dictionary<ConversionQuery, ConversionType>(new ConversionQueryComparer());

        private static readonly Dictionary<ConversionQuery, MethodInfo[]> userConversionMethodsCache =
            new Dictionary<ConversionQuery, MethodInfo[]>(new ConversionQueryComparer());
        ...
        private static bool ExpectsString(in Type destination)
        {
        ...
        {
            return implicitNumericConversions.ContainsKey(source) &&
                   implicitNumericConversions[source].Contains(destination);
        }
        ...
        {
            return explicitNumericConversions.ContainsKey(source) &&
                   explicitNumericConversions[source].Contains(destination);
        }
        ...
        {
            return HasImplicitNumericConversion(source, destination) ||
                   HasExplicitNumericConversion(source, destination);
        }
        ...
        private static ConversionType GetUserDefinedConversionType(in Type source, Type destination)
        {
            ...
            else if (destination.IsPrimitive && destination != typeof(IntPtr) && destination != typeof(UIntPtr))
            {
                ...
                conversionMethod =
                    conversionMethods.FirstOrDefault(m => HasImplicitNumericConversion(m.ReturnType, destination));
                ...
                else
                {
                    conversionMethod =
                        conversionMethods.FirstOrDefault(m =>
                            HasExplicitNumericConversion(m.ReturnType, destination));
        ...
        private static bool IsValidConversion(in ConversionType conversionType, in bool guaranteed)
        {
            ...
            return conversionType != ConversionType.Downcast;
        }
        ...
        /// 可转化为
        /// </summary>
        public static bool IsConvertibleTo(in Type source, in Type destination, in bool guaranteed)
        {
        ...
        /// 可转化为
        /// </summary>
        public static bool IsConvertibleTo(in object source, in Type type, in bool guaranteed)
        {
        ...
        /// 可转化为
        /// </summary>
        public static bool IsConvertibleTo<T>(in object source, in bool guaranteed)
        {
        ...
        /// 转化为
        /// </summary>
        public static object ConvertTo(in object source, in Type type)
        {
        ...
        /// 转化为
        /// </summary>
        public static T ConvertTo<T>(in object source)
        {
        ...
        public static ConversionType GetRequiredConversion(in Type source, in Type destination)
        {
            ...
            if (conversionTypesCache.TryGetValue(query, out var conversionType)) return conversionType;
        ...
        private static ConversionType DetermineConversionType(in ConversionQuery query)
        {
            ...
            if (source == null)
            {
                return destination.IsNullable() ? ConversionType.Identity : ConversionType.Impossible;
            }
        ...
        private static object UserDefinedConversion(in ConversionType conversion, in object value, in Type type)
        {
            ...
            if (numeric)
            {
                ...
                {
                    if (!HasNumericConversion(m.ReturnType, type)) continue;
                    conversionMethod = m;
                    break;
                }
            }
        ...
        private static object Convert(in object value, in Type type, in ConversionType conversionType)
        {
            ...
            if (conversionType == ConversionType.Impossible)
            {
                throw new InvalidConversionException(
                    $"Cannot convert from '{value?.GetType().ToString() ?? "null"}' to '{type}'.");
            }
            ...
            {
                throw new InvalidConversionException(
                    $"Failed to convert from '{value?.GetType().ToString() ?? "null"}' to '{type}' via {conversionType}.",
                    ex);
            }
        }

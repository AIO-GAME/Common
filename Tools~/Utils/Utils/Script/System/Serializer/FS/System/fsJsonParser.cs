...
namespace AIO
{
    ...
    {
        private fsJsonParser(in string input)
        {
            _input = input;
            _start = 0;
        }
        ...
        private fsResult MakeFailure(in string message)
        {
        ...
        private bool HasValue(in int offset)
        {
        ...
        private fsResult TryParseExact(in string content)
        {
            foreach (var item in content)
            {
                if (Character() != item)
                    return MakeFailure("Expected " + item);
        ...
        private bool IsSeparator(in char c)
        {
        ...
        private fsResult TryParseNumber(out fsData data)
        {
            ...
            {
                if (double.TryParse(numberString, NumberStyles.Any, CultureInfo.InvariantCulture, out var doubleValue) == false)
                {
                    data = null;
            ...
            {
                if (long.TryParse(numberString, NumberStyles.Any, CultureInfo.InvariantCulture, out var intValue) == false)
                {
                    data = null;
        ...
        private fsResult TryParseString(out string str)
        {
            ...
            while (HasValue() && Character() != '\"')
            {
                ...
                if (c == '\\')
                {
                    var fail = TryUnescapeChar(out var unescaped);
                    if (fail.Failed)
                    {
                        str = string.Empty;
                        return fail;
        ...
        private fsResult TryParseArray(out fsData arr)
        {
            ...
            {
                // parse the element
                var fail = RunParse(out var element);
                if (fail.Failed)
                {
                    arr = null;
                    return fail;
        ...
        private fsResult TryParseObject(out fsData obj)
        {
            ...
            while (HasValue() && Character() != '}')
            {
                // parse the key
                SkipSpace();
                var failure = TryParseString(out var key);
                if (failure.Failed)
                {
                    obj = null;
                    return failure;
                ...
                // parse the value
                failure = RunParse(out var value);
                if (failure.Failed)
                {
                    obj = null;
                    return failure;
        ...
        private fsResult RunParse(out fsData data)
        {
            ...
            switch (Character())
            {
                ...
                case '"':
                {
                    var fail = TryParseString(out var str);
                    if (fail.Failed)
                    {
                        data = null;
                        return fail;
        ...
        public static fsResult Parse(in string input, out fsData data)
        {
        ...
        /// be recovered.
        /// </summary>
        public static fsData Parse(in string input)
        {
        ...
        private uint ParseSingleChar(in char c1, in uint multipliyer)
        {
            uint p1 = 0;
            if (c1 >= '0' && c1 <= '9')
            {
        ...
        private uint ParseUnicode(in char c1, in char c2, in char c3, in char c4)
        {
